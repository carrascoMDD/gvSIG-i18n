...
cXMLAttributeName_PloneTitle = 'title'
...
class ModelDDvlPloneTool_Export:
    ...
    def fExport( self,
        ...
        # ##############################################################################
        """Encode inthe output encoding, and cache, all meta_type and attribute names.
        ...
        unXMLDocument.encoding = theOutputEncoding
        unExportContext.update( {            
        ...
        unXMLString = unXMLDocument.toprettyxml( encoding = None)
        
        unXMLFileName = theObject.getId() + cXMLFilePostfix
        
        unZipFile.writestr( unXMLFileName, unXMLString)

        unZipFile.close()  
        
        unZIPFileName = theObject.getId() + cZIPFilePostfix
    ...
    def fExport_Recursive( self,
        ...
        if not unTypeConfig:
            """Ignore object and do not export it, allowing cIaller to continue with other objects, if any.
                    ...
                    if not unAttrNameEncoded:
                        ...
                        if unAttrAccessorName:
                            ...
                            if not unAccessor:
                                ...
                                unObjectAttributeFieldType = unAttrConfig.get( 'type', '')
                                try:
                            ...
                            if unObjectSchema.has_key( unAttrName):
                        ...
                        if unObjectAttributeFieldType in [ 'string', 'text', 'selection',]:  
                            ...
                            unaImageAsFile = unRawValue
                            unImageFullFileName = '/'.join( theObject.getPhysicalPath() + ( theObject.getFilename(), ))
