...
class XMIClass(XMIElement, StateMachineContainer):
    ...
    def ORIG_getGenChildren(self, recursive=0):
    ...
    def ORIG_getGenChildrenNames(self, recursive=0):
        """Returns the names of the generalization children."""
        return [o.getName() for o in self.getGenChildren(recursive=recursive)]
    
# OJO ACV CHANGE : avoid repeated children if found through multiple paths

    def getGenChildren(self, recursive=0):
        unosChildren = []
        self.getGenChildrenInto( unosChildren, recursive=recursive)
        return unosChildren
    ...
    def getGenChildrenInto(self, theChildren, recursive=0):
        """Return the generalization children."""
        log.debug("Finding this class's children...")
        
        for r in self.genChildren:
            if not (r in theChildren):
                theChildren.append( r)
        
                if recursive:
                    log.debug("Also looking recursively further down "
                      "the family tree.")  
                    
                    r.getGenChildrenInto( theChildren, 1)           
        return theChildren


# OJO ACV CHANGE : avoid repeated parents if found through multiple paths

    def ORIG_getGenParents(self, recursive = 0):
    ...
    def getGenParents(self, recursive=0):
        unosParents = []
        self.getGenParentsInto( unosParents, recursive=recursive)
        return unosParents
    
    
    
    def getGenParentsInto(self, theParents, recursive=0):
        """Return the generalization children."""
        log.debug("Finding this class's parents...")
        
        for r in self.genParents:
            if not (r in theParents):
                theParents.append( r)
                ...
                if recursive:
                    ...
                    r.getGenParentsInto( theParents, 1)           
        return theParents
