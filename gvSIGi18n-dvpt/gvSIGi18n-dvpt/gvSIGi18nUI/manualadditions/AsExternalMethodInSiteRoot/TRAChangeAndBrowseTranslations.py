...
def _pInitTranslationsCache( 
    ...
    someDomainsStringsAndDefaultsToTranslate = [
        [ 'gvSIGi18n', [    
            ...
            [ 'gvSIGi18n_ToAvoidConfirmationDialogsCheckDoNotConfirmOptionMsg', 'To avoid confirmation dialogs, check the Do not confirm ... box in Options-',],
...
def _pRenderStyles( anOutput, theContextualObject):
    ...
    aRequestUserAgent=theContextualObject.fHTTPRequest_get( "HTTP_USER_AGENT", "").lower()
    if ( "chrome" in aRequestUserAgent) or ( "opera" in aRequestUserAgent)  or ( "safari" in aRequestUserAgent):
        
        anOutput.write( """
            
                       
            <!-- #################################################################
            SECTION: Styles to add presentation and behavior to the page elements for browser Chrome, Opera or Safari
            ################################################################# -->
        ...
        anOutput.write( """
            ...
            <!-- #################################################################
            SECTION: Styles to add presentation and behavior to the page elements for browser other than Chrome, Opera or Safari
            ################################################################# -->
...
def _pRenderList( 
    ...
    for unosDatosTraduccion in pDatosTraducciones:
        ...
        if pMostrarHistoria:
            unasRowsToSpan += 1
        # rowspan Unsupported by Chrome and Opera
        ...
        anOutput.write( u"""  
            <tr class="%(pClassFila)s"  id="cid_FilaPrimeraDeSimbolo_%(symbol_cell_counter)d">
                <td  class="TRAstyle_Display TRAstyle_Clickable" valign="top" 
                    %(entrar_en_edicion)s 
...
def _pRenderMessages( 
    ...
    anOutput.write( u"""     
        ...
        <font color="White"> 
            ...
            <span id="cTRAId_ToAvoidConfirmationDialogsCheckDoNotConfirmOptionMsg" class="TRAstyle_NoDisplay">%(gvSIGi18n_ToAvoidConfirmationDialogsCheckDoNotConfirmOptionMsg)s</span>
